...
- **判定引擎**: 异步响应分析升级为“证据分层 + 冲突裁决”，结构化 JSON 证据优先。
- **上传验证**: 适配服务端重命名文件名，使用多候选文件名校验 upload_dir 可访问性。
- **可解释性**: 新增判定原因与置信等级输出，扫描结果可直接查看“为何判成功/失败”。
- **版本**: 项目版本提升至 v1.0.5。

### v1.0.4 (2026-03-22)

- **扫描**: 文件参数名下拉可编辑；异步/同步响应分析误判修复；mitmproxy 导入诊断提示。
- **Intruder**: 结果排序与 Render 视图；请求/响应详情与多集合爆破相关修复。
- **代理 / Repeater**: 右键发送到行对齐；放行与列表同步等稳定性改进。
- **测试**: 统一为 `test_all.py`；文档与版本号更新至 v1.0.4。
...
- 修复“清除”按钮字体显示不全的问题 (Issue #3)
- Repeater 模块增加标签页拖拽排序和重命名功能 (Issue #2)
- 优化 Webshell 生成代码，移除冗余缩进 (Issue #1)
- 界面细节优化
